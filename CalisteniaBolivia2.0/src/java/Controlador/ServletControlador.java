...
//            }
    ...
    public boolean contiene(int numero, int[] lista){
        //List[] permitidos = new List[20];
    ...
    public int contar(int[] lista, int elem){
        int cant = 0;
        for(int i = 0; i < lista.length; i++){
            if(elem == lista[i]){
                cant++;
    ...
    private void registrarVentaCali2(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException
{
    ...
    String button3 = request.getParameter("btnVentaS");
    if(button1 != null){
    ...
    int idCaja = (Integer.parseInt(request.getParameter("txtIdCajaC").toUpperCase()));
    int idSucursal = CajaBD.getIdSucursalCaja(idCaja);
    int sucur = 0;
    if(idSucursal == 2){
        sucur = 166;
    }else{
    ...
    int[] listaPaquete = new int[50];
    int cont = 0;
    for(contratoCali elem : detalle){
        int id = elem.getIdPaquete();
        listaPaquete[cont] = id;
        cont++;
    }
    int[] ids = new int[20];
    int jj = 0;
    //String pru = "";
    ArrayList<detalleFactura> detalleF = new ArrayList();
    for(contratoCali elem : detalle){
        Paquete p = elem.getPaquete();
        int id = p.getIdPaquete();
        if(contiene(id,ids)==false) {
            int cantidad = contar(listaPaquete,id);
            String articulo = p.getNombrePaquete();
            float precioUnitario = p.getPrecioPaquete();
            detalleFactura df = new detalleFactura(articulo,cantidad,precioUnitario);
            detalleF.add(df);
            ids[jj] = p.getIdPaquete();
            jj++;
        }
        //p.getNombrePaquete();
        //p.getPrecioPaquete();
        ...
        out.println("<script src=\"js/jquery.min.js\"></script>");
                            ...
                            out.println("<script>");
                            out.println("$(document).ready(function(){");
                            out.println("swal ( 'Por favor introduzca datos en Cliente/NIT', '','success' );");
                            out.println("});");
                            out.println("</script>");
    } else {
    rpta=CompraMembresiaBD.CompraMembresia3(cxcm,icxc,i,detalle);
        System.out.println(rpta);
    }
    if(rpta)
    {
                URL url = new URL("http://localhost:80/API_RESTC/factura2");
                //URL url = new URL("http://apirest.calisteniaboliviasc.com/API_RESTC/factura2");
		...
		conn.setRequestProperty("Accept", "application/json");
                ...
                String lista = "";
                for(int ii = 0; ii < detalleF.size(); ii++){
                    detalleFactura detf = detalleF.get(ii);
                    String art = detf.getArticulo();
                    int cant = detf.getCantidad();
                    float pu = detf.getPrecioUnitario();
                    if(ii == 0){
                        lista = "{\"articulo\":\""+art+"\",\"cantidad\":"+cant+",\"precioUnitario\":"+pu+"}";
                    }else{
                        lista = lista + ",{\"articulo\":\""+art+"\",\"cantidad\":"+cant+",\"precioUnitario\":"+pu+"}";
                    }
                }
                
                //String lista = "{\"articulo\":\"Membresia\",\"cantidad\":2,\"precioUnitario\":350},{\"articulo\":\"Membresia 2\",\"cantidad\":1,\"precioUnitario\":300}";
                //String input = "{\"emisor\":\"308210028\",\"comprador\":\"123\",\"razonSocial\":\"razon_social_test\",\"listaItems\":["+lista+"],\"descuentos\":0,\"tipoCompra\":1}";
                //String input = "{\"emisor\":\"308210028\",\"comprador\":\""+nit+"\",\"razonSocial\":\""+cliente+"\",\"listaItems\":["+lista+"],\"descuentos\":0,\"tipoCompra\":1}";
                 String input = "{\"emisor\":\"308210028\",\"comprador\":\""+nit+"\",\"razonSocial\":\""+cliente+"\",\"listaItems\":["+lista+"],\"descuentos\":0,\"tipoCompra\":1,\"sucursal\":"+sucur+"}";
                String json = new Gson().toJson(input);
                System.out.println(json);
                conn.getOutputStream().write((input).getBytes(StandardCharsets.UTF_8));
                conn.connect();
...
while ((bre.readLine()) != null) {
		...
		//}
                ...
                boolean rpta2=CompraMembresiaBD.modificarNroFactura(id);
		...
		conn.disconnect();            
                ...
                /*Process p = Runtime
			   .getRuntime()
			   .exec("rundll32 url.dll,FileProtocolHandler "+output);
			p.waitFor();*/
        ...
        out.println("<script src=\"js/jquery.min.js\"></script>");
                            ...
                            out.println("<script>");
                            out.println("$(document).ready(function(){ "); 
                            out.println("var myWindow = window.open(\""+output+"\",\"ventana1\");");
                            out.println("});");
                            out.println("</script>");
    ...
    private void registrarVentaBox(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException
{
    ...
    rpta=CompraMembresiaBD.CompraMembresiaBox(cxcm,icxc,i,detalle,rc);
        System.out.println(rpta);
    }
    if(rpta)
    {
...
while ((bre.readLine()) != null) {
		...
		String output;
                ...
                boolean rpta2=CompraMembresiaBD.modificarNroFactura(id2);
