...
public class PaqueteVenta
{
    ...
    public void registro(JSONObject obj, SSSessionAbstract session) {
        try {
                        ...
                        try {
                            ...
                            if (JSONObject.getNames(data)[j].equals("2") || JSONObject.getNames(data)[j].equals("3")) {
                                if (JSONObject.getNames(data)[j].equals("3")) {
                                    String codigo = data.getJSONObject(JSONObject.getNames(data)[j]).getString("C\u00f3digo");
            ...
            mail.put("__KEY_PAQUETE__", (String.valueOf(paquete_venta.getString("key_paquete")) + "?fecha=" + new Date().toString()));
            for (int k = 0; k < clientes.length(); ++k) {
                ...
                mail.put("__KEY_USUARIO_CLIENTE__", (String.valueOf(paquete_venta_usuarios.getJSONObject(k).getString("key")) + "?fecha=" + new Date().toString()));
        ...
        try {
            ...
            if (!paquetes_venta_usuario.isEmpty()) {
                for (int i = 0; i < JSONObject.getNames(paquetes_venta_usuario).length; ++i) {
                    JSONObject paquete_venta_usuario = paquetes_venta_usuario.getJSONObject(JSONObject.getNames(paquetes_venta_usuario)[i]);
                    usuarios.put(paquete_venta_usuario.getString("key_usuario"));
                    JSONObject caja_movimientos = CajaMovimiento.getMovimientosVentaServicio(paquete_venta_usuario.getString("key_caja"), paquete_venta_usuario.getString("key"));
                    if (!caja_movimientos.isEmpty()) {
                        for (int j = 0; j < JSONObject.getNames(caja_movimientos).length; ++j) {
                            ...
                            if (caja_movimiento.getString("key_tipo_pago").equals("2") || caja_movimiento.getString("key_tipo_pago").equals("3")) {
                                JSONObject cuentaBancoMovimiento = CuentaBancoMovimiento.getByKeyCajaMovimiento(key_caja_old);
