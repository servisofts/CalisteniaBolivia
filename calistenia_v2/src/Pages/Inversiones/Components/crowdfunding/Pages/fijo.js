...
class fijo extends Component {
  constructor(props) {
    ...
    // ventana que presionar arribar, subir cuadrado
  }

  ItemDetalle(key_inversion, precio_accion, datafiltrada) {
    var usuarios = Model.usuario.Action.getAll();
    if (!usuarios) return <SLoad />;

    return datafiltrada.map((obj) => {
      let cantidad_interes = this.state.select[key_inversion] ?? 0;
        ...
        <SView col={"xs-12"} backgroundColor={"cyan+66"}>
          <SView col={"xs-12"} backgroundColor={"cyan+66"} row>
            ...
            <SView
              ...
              border={"transparent"}
            >
              <SText fontSize={10} col={"xs-12"} row center>
                {_accion}
              </SText>
            </SView>
            <SView
            ...
            >
              <SText fontSize={10}>
                {SMath.formatMoney(obj.inversion, 0)} Bs
              </SText>
            </SView>
            ...
            >
              <SText fontSize={10} col={"xs-12"} row center>
                x{SMath.formatMoney(cantidad_interes, 0)}
              </SText>
            </SView>
  ...
  Item(obj) {
    ...
    let datafiltrada = Object.values(datax).filter(
      (a) => a.key_fondo_inversion == obj.inversion_key
    );

    if (
      obj.inversion_key != "010110c7-6580-4184-988c-f75fc2e86ac1" &&
      obj.inversion_key != "1120d5c7-f7e4-427a-978f-2c238668a6e4"
    )
      return;
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            <SText center fontSize={14} bold>
              {obj?.descripcion}
            </SText>
            <SHr height={8} />
            <SText center fontSize={12}>
              {obj?.observacion}
            </SText>
            <SHr height={8} />
            <SText center fontSize={12}>
              Fecha {obj?.fecha_inicio + " - " + obj?.fecha_fin}
            </SText>
              ...
              <SView col={"xs-3"} height center>
                ...
                >
                  <SText fontSize={14} bold>
                    {SMath.formatMoney(obj?.monto_maximo)}
                  </SText>
                </SView>
                <SHr height={2} />
                <SText center fontSize={10}>
                  Monto maximo
                </SText>
              </SView>
              <SView col={"xs-3"} height center>
              ...
              <SView col={"xs-3"} height center>
                ...
                >
                  <SInput
                    ...
                    onChangeText={(txt) => {
                      this.state.select[obj.inversion_key] = txt;
                      this.setState({ ...this.state });
                    }}
                  />
                </SView>
                <SHr height={2} />
                <SText col={"xs-12"} row center fontSize={10}>
                  Interes
                </SText>
              </SView>
            </SView>
          ...
          <SView col={"xs-11.8"} backgroundColor={"cyan+66"}>
            <SView col={"xs-12"} row center>
              ...
              >
                <SText fontSize={10}>
                  # {SMath.formatMoney(datafiltrada.length, 0)}
                </SText>
              </SView>
            </SView>
            <SHr height={2} color={STheme.color.card} />
            <SView col={"xs-12"} backgroundColor={"cyan+66"} card row>
              ...
              <SView
                ...
                style={{ justifyContent: "flex-end" }}
              >
                <SText fontSize={10}>Estado Interes</SText>
              </SView>
            </SView>
          </SView>
          <SHr height={4} />
  ...
  getLista() {
    ...
    return (
      <SList
        ...
        horizontal
        space={0}
        render={(obj_fondo) => this.Item(obj_fondo)}
      />
    );
  }
