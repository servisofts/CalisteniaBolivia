...
class NavBar extends React.Component {
	...
	renderUserData() {
		...
		if (!usuario) return <SView col={"xs-12"} center height onPress={() => {
			...
			<SText color={STheme.color.secondary} fontSize={18} center>{"Inicia session en tapeke."}</SText>
		...
		return <SView col={"xs-9 md-6 xl-4"} height animated backgroundColor={STheme.color.background} style={{
			...
			<SScrollView2 disableHorizontal >
				<SView col={"xs-12"} center  >
					...
					{this.renderIcon({ label: "Mis compras", icon: "Compras", path: "/compras", requireUser: true })}
					{this.renderIcon({ label: "Mis cupones", icon: "Billetera", path: "/cupones", requireUser: true })}
					{this.renderIcon({ label: "Mi billetera", icon: "Billetera", path: "/billetera", requireUser: true })}
					{this.renderIcon({ label: "Notificaciones", icon: "KNotify", path: "/notificaciones", requireUser: true })}
